...
class UserController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        $request->validate(
            ...
            [
                'phone_number' => 'Cannot Proceed With this Phonenumber',
            ]
        );
        try {
        ...
        if ($sendmail) {
            ...
            return response()->json(
                [
                    "toaster_success" => $fullname . ",Check Your Email And Set The  Password"
                ],
                201
            );
        } else {
            return response()->json(
                [
                    "toaster_error" => "Oops Something Went Wrong!"
                ],
                422
            );
        }
    ...
    public function password_page(Request $request)
    {
        ...
        if ($user) {
            return response()->json(
                [
                    "toaster_info" => "Set Your Password"
                ],
                200
            );
        } else {
            return response()->json(
                [
                    "message" => "Invalid User Error Occured"
                ],
                422
            );
        }
    ...
    public function save_password(Request $request)
    {
        ...
        if ($password_saved) {
            return response()->json(
                [
                    "toaster_success" => "Password SetUp Succesful"
                ],
                200
            );
        } else {
            return response()->json(
                [
                    "toaster_error" => "Expired, Contact The Admin"
                ],
                422
            );
        }
